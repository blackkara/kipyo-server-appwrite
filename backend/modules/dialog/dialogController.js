...
class DialogController {
  ...
  async deleteDialog(req, res) {
    ...
    try {
      ...
      const { occupantId, blockAlso } = req.body;

      log(`[${requestId}] Request params: occupantId=${occupantId}, blockAlso=${blockAlso}, requesterId=${requestedUser.$id}`);
  ...
  async allowMedia(req, res) {
    const { startTime, requestId, jwtToken, requestedUser } = req;
    const log = (message) => console.log(message);
    const error = (message, err) => console.error(message, err);

    try {
      log(`[${requestId}] Allow media request started`);
      const { dialogId, allowed } = req.body;

      log(`[${requestId}] Request params: dialogId=${dialogId}, requesterId=${requestedUser.$id}`);
      const result = await dialogService.allowMedia(
        jwtToken,
        requestedUser.$id,
        dialogId,
        allowed,
      ...
      return res.status(200).json({
        success: true,
        code: 200,
        message: 'Media permissions changed successfully',
        data: result,
        requestId: requestId,
        duration: duration
      });
